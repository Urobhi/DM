...
cppFunction('NumericVector fhistC(NumericVector pcolumna, IntegerVector pdesde ) 
{
 ...
 if ( !R_IsNA (pcolumna[i-2]))
  {
    out[ i + 5*n ]  =  pcolumna[i-1];
  }
...
dataset <- fread( "./datasetsOri/paquete_premium2.txt.gz") 
...
dataset[ foto_mes==201905,  ctarjeta_visa_debitos_automaticos  := NA ]#  No veo porque esto
...
# Proporción de dlares en la cuenta
dataset[ , rcajaahorro_dolarestotal   :=  mcaja_ahorro_dolares/mcuentas_saldo ]
dataset[ , rplazofijo_dolarestotal    :=  mplazo_fijo_dolares/mcuentas_saldo ]
dataset[ , rinv1_dolarestotal         :=  minversion1_dolares/mcuentas_saldo ]
dataset[ , rplazofijo_dolares         :=  mplazo_fijo_dolares/rowSums(cbind(mplazo_fijo_dolares,mplazo_fijo_pesos), na.rm = TRUE)]
dataset[ , rinversion1_dolarestotal   :=  minversion1_dolares/rowSums(cbind(minversion1_dolares,minversion1_pesos), na.rm = TRUE)]


# Medidas de solvencia
dataset[ , r_activos_pasivos       := rowSums(cbind(mcuentas_saldo,mplazo_fijo_dolares,mplazo_fijo_pesos,minversion1_dolares,minversion1_pesos,minversion2), na.rm=TRUE ) / pmax(rowSums(cbind(mprestamos_personales,mprestamos_prendarios,mprestamos_hipotecarios), na.rm=TRUE),0.01)] 
...
# Que porcentaje del consumo es el pago mínimo?
dataset[ , mvr_mpagominimo_consumo         := mv_mpagominimo  / mv_mconsumototal ]
...
fwrite( dataset,
        ...
        sep= "\t" )
save( dataset,
		file="./datasets/paquete_premium_ext_corregido.RData")
...
#columnas_no_procesar  <- c( "numero_de_cliente", "foto_mes", "clase_ternaria","internet","cliente_edad","cliente_antiguedad","tpaquete2","tpaquete3","tpaquete4","ccuenta_corriente","ccaja_ahorro","ctarjeta_debito","ctarjeta_visa","ctarjeta_master","cprestamos_personales","cprestamos_prendarios","cprestamos_hipotecarios","cplazo_fijo","cinversion1","cinversion2","cpayroll_trx","cpayroll2_trx","ccajeros_propios_descuentos","active_quarter","tcuentas","ccomisiones_otras","cforex_buy","cforex_sell","ctarjeta_visa_debitos_automaticos","ctarjeta_master_debitos_automaticos","ccuenta_debitos_automaticos","cpagodeservicios","cpagomiscuentas","ctarjeta_visa_descuentos","ctarjeta_master_descuentos","ccomisiones_mantenimiento","ccomisiones_otras","cforex","ctransferencias_recibidas","ccheques_depositados","cextraccion_autoservicio","tcallcenter","thomebanking","catm_trx","matm","ctarjeta_debito_transacciones","ctarjeta_master_transacciones","ctarjeta_visa_transacciones","mpayroll2","catm_trx_other","Master_status","Master_mfinanciacion_limite","Master_Fvencimiento","Master_Finiciomora","Master_msaldototal","Master_msaldopesos","Master_msaldodolares","Master_mconsumospesos","Master_mconsumosdolares","Master_mlimitecompra","Master_madelantopesos","Master_madelantodolares","Master_fultimo_cierre","Master_mpagado","Master_mpagospesos","Master_mpagosdolares","Master_fechaalta","Master_mconsumototal","Master_cconsumos","Master_cadelantosefectivo","Master_mpagominimo","Visa_delinquency","Visa_status","Visa_mfinanciacion_limite","Visa_Fvencimiento","Visa_Finiciomora","Visa_msaldototal","Visa_msaldopesos","Visa_msaldodolares","Visa_mconsumospesos","Visa_mconsumosdolares","Visa_mlimitecompra","Visa_madelantopesos","Visa_madelantodolares","Visa_fultimo_cierre","Visa_mpagado","Visa_mpagospesos","Visa_mpagosdolares","Visa_fechaalta","Visa_mconsumototal","Visa_cconsumos","Visa_cadelantosefectivo","Visa_mpagominimo")
#columnas_extendidas_a_procesar  <- setdiff( colnames(dataset),  columnas_no_procesar)
columnas_extendidas_a_procesar  <- setdiff( colnames(dataset),  c("clase_ternaria","numero_de_cliente"))  
...
for(  campo  in  columnas_extendidas_a_procesar )
{
   ...
   dataset[ , paste( campo, "__lag2" , sep="" ):= nueva_col[ (5*last +1):(6*last) ]  ]
   dataset[ , paste(campo,"__diflag", sep="" ):=get(campo) - get(paste( campo,"__lag",sep=""))]
   dataset[ , paste(campo,"__diflag2", sep="" ):=get(campo) - get(paste( campo,"__lag",sep=""))]
}
...
fwrite( dataset[ foto_mes>=201902 & foto_mes<=202006 , ] , 
        file= "./datasets/paquete_premium_2_exthist_corregido_lag2_diff_todo_cpp.txt.gz", 
        sep="\t" )
