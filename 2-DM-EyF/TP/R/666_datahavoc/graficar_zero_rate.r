...
{
  tbl <- dataset[ foto_mes<=202001 ,  list( "zero_ratio" = sum(get(campo)==0, na.rm=TRUE)/.N ) , foto_mes ]
  ...
  abline(v=c(34), col=c("red"), lty=c(1), lwd=c(1))
...
for( campo in  campos_buenos )
{
  tbl <- dataset[ foto_mes<=202001 ,  list( "na_ratio" = sum( is.na(get(campo)), na.rm=TRUE)/.N ) , foto_mes ]
...
for( campo in  campos_buenos )
{
  tbl   <- dataset[ foto_mes<=202001 ,  list( "promedio" = mean( get(campo), na.rm=TRUE)) , foto_mes ]
  ceros <- dataset[ foto_mes<=202001 ,  list( "zero_ratio" = sum(get(campo)==0, na.rm=TRUE)/.N ) , foto_mes ]
...
for( campo in  campos_buenos )
{
  tbl   <- dataset[ foto_mes<=202001 & get(campo)!=0,  list( "promedio" = mean( get(campo), na.rm=TRUE)) , foto_mes ]
  ceros <- dataset[ foto_mes<=202001 ,  list( "zero_ratio" = sum(get(campo)==0, na.rm=TRUE)/.N ) , foto_mes ]
...
dataset[ foto_me
s<=202001 ,  list( "promedio" = mean( mcaja_ahorro_dolares, na.rm=TRUE)) , foto_mes ]
dataset[ foto_mes<=202001 ,  list( "zero_ratio" = sum(chomebanking_transacciones==0, na.rm=TRUE)/.N ) , foto_mes ]
